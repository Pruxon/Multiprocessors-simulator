...
PUBLIC unsigned short is_proc_ready1(rp)
struct proc * rp;
{
	struct proc * xp;
	xp = rdy_head[PRI_Q1];
...
PUBLIC unsigned short is_proc_ready2(rp)
struct proc * rp;
{
	struct proc * xp;
	xp = rdy_head[PRI_Q2];
 ...
 */
 ...
 if ( (rp = rdy_head[TASK_Q]) != NIL_PROC) {
  ...
  if ( (rp = rdy_head[PRI_Q1]) != NIL_PROC) {
	proc_ptr = rp;
	bill_ptr = rp;
	return;
  }
  if ( (rp = rdy_head[PRI_Q2]) != NIL_PROC) {
	proc_ptr = rp;
	bill_ptr = rp;
	return;
  }
 ...
 */
  ...
  if(rdy_head[PRI_Q1] == NIL_PROC)
	...
	rdy_tail[PRI_Q1]->p_nextready = rp;
	rdy_tail[PRI_Q1] = rp;
	rdy_tail[PRI_Q1]->p_nextready = NIL_PROC;
  }
  else if (rp->id_grupy == 3)
  {
  if (rdy_head[PRI_Q2] == NIL_PROC)
	{
	rdy_tail[PRI_Q2] = rp;
	rdy_head[PRI_Q2] = rp;
	}
	...
	{
	struct proc *tmp1;
	tmp1 = rdy_head[PRI_Q2];
	if (rdy_head[PRI_Q2]->time >= rp->time)
	{
		rp->p_nextready = rdy_head[PRI_Q2];
		rdy_head[PRI_Q2] = rp;
	}
	else
	{
		...
		{	
			if (rp->time < tmp1->p_nextready->time){
			rp->p_nextready = tmp1->p_nextready;
			tmp1->p_nextready = rp;
			return;
			}
		tmp1 = tmp1->p_nextready;
		}
	rdy_tail[PRI_Q2]->p_nextready = rp;
	rdy_tail[PRI_Q2] = rp;
	}
  }
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	if ( (xp = rdy_head[PRI_Q1]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[PRI_Q1] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
	...
	qtail = &rdy_tail[PRI_Q1];
  } else if (rp->id_grupy == 3){
	if ( (xp = rdy_head[PRI_Q2]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[PRI_Q2] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC && rdy_head[PRI_Q1] == NIL_PROC && rdy_head[PRI_Q2] == NIL_PROC) return;
  ...
  else if (rdy_head[PRI_Q1] != NIL_PROC)
  {
	struct proc *temp;
	if (rdy_head[PRI_Q1]->p_nextready->time < 1000)
	{
		for (temp = rdy_head[PRI_Q1]; temp->p_nextready != NIL_PROC; temp = temp->p_nextready)
			temp->p_nextready->time = (temp->p_nextready->time <= 1000) ? (temp->p_nextready->time + 1 ):temp->p_nextready->time;
		pick_proc();
		return;
	}
	else
	{
		rdy_head[PRI_Q1]->time = 1;
		rdy_tail[PRI_Q1]->p_nextready = rdy_head[PRI_Q1];
		rdy_tail[PRI_Q1] = rdy_head[PRI_Q1];
		rdy_head[PRI_Q1] = rdy_head[PRI_Q1]->p_nextready;
		rdy_tail[PRI_Q1]->p_nextready = NIL_PROC;
		pick_proc();
		return;
	}
