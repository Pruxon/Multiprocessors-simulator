...
FORWARD _PROTOTYPE( int do_setid, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getid, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETID:	r = do_setid(&m);	break;
	    case SYS_GETID:	r = do_getid(&m);	break;
...
PRIVATE int do_setid(m_ptr)
message * m_ptr;
{
	struct proc * rp;
	int new_id = m_ptr->m2_l2;
...
{
	struct proc * rp;
	int new_time = m_ptr->m2_l1;
	
	if((rp = find_user_proc_by_pid(m_ptr->m1_i1)) == NIL_PROC)
		return EINVAL;
	
	if(new_time < 1 || new_time > 211)
		return EINVAL;
